...
> 项目周期：8周  
> 团队规模：2人  
> 当前阶段：第1周——网页/PWA、后端基础框架和第一个真实视觉模型已完成，下一步接入目标检测并建立固定场景
...
| 自动化与浏览器检查 | 已完成 | 36项测试通过，`app`与`core`覆盖率91%；真实视觉API、桌面、手机、离线、更新、键盘和错误上传流程已验证 |
| 真实检测、OCR和视觉理解模型 | 进行中 | 智谱GLM-4.5V Provider已接入并通过真实调用；目标检测和独立OCR仍需选型、登记和接入 |
| 基础端到端流程 | 已完成 | 可上传图片、分析质量、返回真实系统状态并使用浏览器语音播报 |
| AI功能MVP | 进行中 | 已完成单次“上传图片→视觉理解→结构化证据→网页结果”真实流程；待检测、OCR、固定测试和连续演示 |
...
| 视觉理解 | 智谱GLM-4.5V API | 场景概述、文字读取回退、物品查找和追问 | 已接入；结构化提示和安全约束已实现，需评测幻觉、成本和网络依赖 |
...
|---|---|---|---|---|---|
| 第1周 | 完成Git、Python环境、OpenCV和基础工程；接入视觉API并启动目标检测基线 | 整理5份权威资料、竞品功能和20个使用场景 | 冻结核心场景、P0范围、不做事项和验收标准 | 需求依据表、视觉演示、功能边界 | 进行中：工程和视觉API已完成，检测基线待完成 |
...
| 2 | 比较2—3个轻量目标检测候选并实现真实检测Provider | 许可证登记完成；上传图片可返回物体框、类别、置信度和耗时 |
