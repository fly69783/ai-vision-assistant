...
| 目标检测、OCR、视觉问答、TTS | 未开始 | 需要分别跑通最小示例 |
| MVP完整流程 | 未开始 | 尚未形成可演示版本 |
...
| 模块 | 初步方案 | 主要作用 | 主要风险 |
|---|---|---|---|
| 图片和摄像头 | OpenCV | 读取、缩放和检查画面质量 | 低光、模糊和摄像头兼容问题 |
| 目标检测 | 轻量YOLO | 找到人、椅子、桌子、杯子等物体 | 门、台阶等类别可能需要补充数据 |
| OCR | PaddleOCR轻量模型 | 读取中文场景文字 | 小字、反光和倾斜文字可能出错 |
| 视觉理解 | 视觉语言模型 | 场景概述、关系理解和追问 | 可能产生幻觉或回答过长 |
| 结果融合 | 自定义规则 | 结合用户意图、置信度和信息新颖度排序 | 规则需要通过测试不断调整 |
| 语音 | 系统TTS或本地TTS | 播放、停止和重复结果 | 播报过长会干扰用户 |
| 后端接口 | FastAPI | 接收图片并返回结构化结果 | 超时和网络失败需要降级 |
| 前端载体 | 网页或Android | 摄像头、主按钮和语音交互 | 第1周内确定最低成本方案 |
...
| 角色 | 主要负责人 | 具体职责 | 每周证据 |
|---|---|---|---|
| 技术主力 | 成员A（你） | Python工程、OpenCV、YOLO、OCR、视觉模型、结果融合、后端、性能和技术答辩 | 可运行版本、演示截图、性能数据、3个失败案例 |
...
|---|---|---|---|---|---|
| 第1周 | 掌握Git、Python环境和OpenCV；跑通图片读取与目标检测示例 | 整理5份权威资料、竞品功能和20个使用场景 | 冻结3个核心场景、P0范围、不做事项和验收标准 | 需求依据表、检测演示、功能边界 | 进行中 |
| 第2周 | 分别跑通OCR、TTS和视觉问答示例，记录耗时和错误 | 收集并整理40个合法测试样本，完成界面线框图 | 确定统一结果格式和播报规范 | 5个模块示例、样本清单、界面草图 | 未开始 |
| 第3周 | 建立FastAPI接口，统一各模块的输入输出和错误格式 | 编写正常、模糊、低光、遮挡和断网测试用例 | 完成“上传图片→分析→语音播报”最小流程 | 第一版端到端流程、测试表 | 未开始 |
...
| 跑通OpenCV读取图片 | 技术主力 | 能读取、显示或保存一张图片 | 未完成 |
| 跑通轻量YOLO图片检测 | 技术主力 | 能输出框、类别和置信度 | 未完成 |
