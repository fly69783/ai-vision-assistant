...
> [!IMPORTANT]
> **当前已完成可运行的网页/PWA和后端基础框架，但尚未接入真实AI模型，因此还不是比赛MVP。** 现在可以在手机拍照或从相册选图，检查画面质量、选择任务、调用统一API并听取浏览器语音提示；目标检测、OCR和视觉理解默认明确显示“未配置”，不会用假结果冒充模型输出。本项目不是导航系统，不得用于道路通行、避障或其他安全决策。
...
| 28项自动化测试和真实浏览器流程检查 | 原生Android/iOS应用 |
...
| 自动化与浏览器验证 | ✅ 已完成 | 28项测试通过，`app`与`core`覆盖率91%；桌面、390×844手机、离线恢复、PWA更新、键盘和错误上传流程通过 |
