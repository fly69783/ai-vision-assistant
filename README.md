...
> [!IMPORTANT]
> **当前已接入智谱GLM-4.5V视觉理解API，可以完成环境概述、文字读取回退、物品查找和视觉追问；目标检测和独立OCR仍待接入，因此还不是完整比赛MVP。** 网页/PWA支持手机拍照或相册选图、画面质量检查、结构化结果和语音播报。没有配置的能力会如实显示“未配置”，不会用假结果冒充模型输出。本项目不是导航系统，不得用于道路通行、避障或其他安全决策。
...
| 已经可以运行 | 仍然没有实现 |
|---|---|
| 可安装、适配手机的高对比度PWA | 真实目标检测模型 |
| 拍照、相册、拖放选图及格式/大小/像素校验 | 真实OCR文字识别 |
| 亮度、模糊度和分辨率检查 | 真实视觉语言模型问答 |
| 四类任务路由和统一JSON接口 | 摄像头连续视频与目标跟踪 |
| GLM-4.5V真实视觉理解与结构化证据 | 经过固定测试集验证的准确率 |
| 浏览器自带语音播报 | 可供视障用户实际依赖的产品能力 |
| 离线外壳、网络恢复和安全响应策略 | 比赛现场可交付的完整MVP |
| 36项自动化测试和真实浏览器AI流程检查 | 原生Android/iOS应用 |

当前版本已经完成第一项真实AI能力。后续接入目标检测和独立OCR时，只需要实现对应的能力适配器，不必重写网页、API、质量检查、结果格式和测试框架。
...
|---|---|---|---|---|
| 意图驱动的可信融合 | 单一模型容易漏检或产生幻觉 | 检测、OCR、视觉模型保留各自来源和置信度，按任务相关性、重要性、置信度、新颖度排序 | 单视觉模型 vs 多模型融合 | 结构和排序服务已实现，待真实模型验证 |
...
| 自动化与浏览器验证 | ✅ 已完成 | 36项测试通过，`app`与`core`覆盖率91%；两次真实GLM-4.5V浏览器检查约2.1—9.9秒完成，既有桌面、手机、离线、更新、键盘和错误上传流程通过 |
| 检测、OCR、视觉理解、TTS独立示例 | 🚧 进行中 | 视觉理解和浏览器TTS已可用；真实检测和独立OCR待接入 |
| 上传图片到语音播报的MVP | 🚧 进行中 | 单次真实端到端流程已通过，待固定图片连续演示3次并记录结果 |
| 固定测试集和评测报告 | ⏳ 待完成 | 建立40个合法测试场景 |
| 用户手册、视频和答辩材料 | ⏳ 待完成 | 第6周起集中整理 |

> 当前 `main.py` 已是正式开发入口。视觉API在环境变量齐全时启用，检测和OCR适配器仍会诚实返回“未配置”。接入一个模型不等于完整AI功能完成；只有固定测试和演示证据齐全后，才能把比赛MVP标记为“已完成”。
